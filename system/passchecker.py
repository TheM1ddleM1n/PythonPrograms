...
def crack_time(entropy_bits):
    ...
    if seconds < 86400 * 365: return "Months"
    return "Years"
...
def check_strength():
    ...
    else: missing.append("an uppercase letter")
    if re.search(r"[a-z]", password): score += 1
    else: missing.append("a lowercase letter")
    if re.search(r"[0-9]", password): score += 1
    else: missing.append("a number")
    if re.search(r"[!@#$%^&*(),.?\":{}|<>]", password): score += 1
    else: missing.append("a special character")
    ...
    result = ""
    result += f"Password Strength Check\n"
    result += f"------------------------\n"
...
text_output = tk.Text(root, height=12, width=60, state="disabled",
                      bg="#001100", fg="#00ff00", font=("Consolas", 11))
