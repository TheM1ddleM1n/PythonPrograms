...
class FileSorter:
    ...
    def _process_file(self, file_path):
        ...
        try:
            ...
            if self.dry_run:
                print(
                    f"Would move: {file_path.name} → {target_folder.relative_to(self.source_path)}"
                )
            else:
                target_folder.mkdir(parents=True, exist_ok=True)
                shutil.move(str(file_path), str(target_path))
                print(
                    f"✅ Moved: {file_path.name} → {target_folder.relative_to(self.source_path)}"
                )
...
Examples:
  ...
  python FileSorter.py ~/Downloads --by-date
    ...
    parser.add_argument("folder", help="Path to folder to sort")
